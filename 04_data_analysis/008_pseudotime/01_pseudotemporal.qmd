...
ggsave(
  here(
    "05_figures/990_shared_figures/004_pseudotime",
    "slingshot_pca_diagnosis-sex.png"
  ),
  width = 7,
  height = 8
)
...
EGF identified as a very interesting growth factor that can induce a proportion of endothelial cells to spontaneously differentiate to pericytes in culter.

```{r}
# Get all human pathways
pathways <- msigdbr(species = "Homo sapiens")

# Search for EGF-related pathways
egf_pathways <- pathways[grepl("EGF", pathways$gs_name, ignore.case = TRUE), ]

# View available pathway names
unique(egf_pathways$gs_name)
length(unique(egf_pathways$gene_symbol))
# check angpt2 out of curiosity
egf_pathways |> filter(gene_symbol == "ANGPT2") |> select(gs_name)

# Get genes from a specific pathway (example)
egf_genes <- pathways[pathways$gs_name == "REACTOME_SIGNALING_BY_EGFR", ] |>
  distinct(gene_symbol, .keep_all = TRUE) |>
  filter(ensembl_gene %in% rownames(sce_subset))
egfr_genes <- pathways[pathways$gs_name == "EGFR_TARGET_GENES", ] |>
  distinct(gene_symbol, .keep_all = TRUE) |>
  filter(ensembl_gene %in% rownames(sce_subset))


plots <- map2(
  egf_genes$ensembl_gene,
  egf_genes$gene_symbol,
  plot_gene_pseudotime,
  sce_subset,
  counts
)
...
plots <- map2(
  egfr_genes$ensembl_gene,
  egfr_genes$gene_symbol,
  plot_gene_pseudotime,
  sce_subset,
  counts
)
...
Harriet also identified some genes of interest related to EGF
...
egf_genes <- translated_id |>
  filter(hgnc_symbol %in% genes) |>
  filter(ensembl_gene_id %in% rownames(sce_subset))

plots <- map2(
  egf_genes$ensembl_gene_id,
  egf_genes$hgnc_symbol,
  plot_gene_pseudotime,
  sce_subset,
  counts
)
...
plot_endomt
# If getting error with fonts when saving plot
# library(extrafont)
# font_import()  # Import system fonts
# loadfonts(device = "pdf")  # Register for PDF device
ggsave(
  here::here(
    "05_figures/990_shared_figures/003_final_figures",
    "endomt_fig3.pdf"
