...
sce_subset_smc <- qs::qread(str_replace(file, "_pericyte", "_smc"))
...
plot_gene_pseudotime <- function(gene, gene_symbol, sce, counts, extra_title = NULL) {
  plot <- plotSmoothers(sce, counts, gene, curvesCols = pal_npg("nrc")(2)) +
    scale_colour_npg(labels = c("EndoMT-PC - AD",
                                "EndoMT-PC - Control")) +
    ggtitle(paste0(if_else(gene_symbol == "", gene, gene_symbol), " - ", extra_title)) +
    labs(colour = "") +
...
counts_smc <- assays(sce_subset_smc)$counts
```
...
cond_res_smc <- conditionTest(sce_subset_smc, lineages = TRUE, pairwise = TRUE)

sum(cond_res$pvalue < 0.05, na.rm = TRUE)
sum(cond_res$pvalue == 0, na.rm = TRUE)
...
pdf(file, width = 12.5, height = 11)  # Adjust page size as needed
...
early_tf <- comprehensive_tf_check(unique(cond_res_sig$hgnc_symbol))
table(early_tf$TF_Sources)
early_tf <- early_tf |>
  dplyr::filter(Is_TF) |>
  dplyr::left_join(translated_id, by = join_by(Gene == hgnc_symbol))
early_tf_smc <- comprehensive_tf_check(unique(cond_res_sig_smc$hgnc_symbol)) |>
  dplyr::filter(Is_TF) |>
  dplyr::left_join(translated_id, by = join_by(Gene == hgnc_symbol))
...
tf_link_match <- cond_res_sig |>
  dplyr::filter(hgnc_symbol %in% tf_link_table$Name.TF & !hgnc_symbol %in% early_tf$Gene)
tf_link_match_smc <- cond_res_sig_smc |>
  dplyr::filter(hgnc_symbol %in% tf_link_table$Name.TF & !hgnc_symbol %in% early_tf_smc$Gene)
...
all_tfs <- data.frame(gene = c(early_tf$Gene, tf_link_match$hgnc_symbol),
  ...
  lineage = "Pericyte")
all_tfs_smc <- data.frame(gene = c(early_tf_smc$Gene, tf_link_match_smc$hgnc_symbol),
  ensembl_gene_id = c(early_tf_smc$ensembl_gene_id, tf_link_match_smc$ensembl_gene_id),
  lineage = "SMC")

all_tfs_joined <- rbind(all_tfs, all_tfs_smc) |>
  mutate(lineage_group = case_when(
    n_distinct(lineage) == 2 ~ "Both",
    lineage[1] == "Pericyte" ~ "Pericyte",
    lineage[1] == "SMC" ~ "SMC"
  ), .by = gene) |>
  dplyr::distinct(gene, .keep_all = TRUE) |>
  dplyr::mutate(plot_title = paste0(gene, " - ", lineage_group))

tfs_lineage_pericyte <- all_tfs_joined |>
  dplyr::filter(lineage_group == "Pericyte")
tfs_lineage_smc <- all_tfs_joined |>
  dplyr::filter(lineage_group == "SMC")

plots_peri <- map2(tfs_lineage_pericyte$ensembl_gene_id,
              tfs_lineage_pericyte$gene,
              plot_gene_pseudotime, sce_subset, counts, "Pericyte")

plots_smc <- map2(tfs_lineage_smc$ensembl_gene_id,
              tfs_lineage_smc$gene,
              plot_gene_pseudotime, sce_subset_smc, counts_smc, "SMC")
plots <- c(plots_peri, plots_smc)
...
file <- here("05_figures/990_shared_figures/004_pseudotime",
                       "01_tradeseq", "tradeseq_tfs_split_lineage.pdf")
...
file <- here("05_figures/990_shared_figures/004_pseudotime",
                       "01_tradeseq", "tradeseq_tfs_pericyte_lineage.pdf")

if(subset != "pericyte"){
  file <- str_replace(file, "_pericyte", "_smc")
  plots <- map2(all_tfs_smc$ensembl_gene_id,
                all_tfs_smc$gene,
                plot_gene_pseudotime, sce_subset_smc, counts_smc)
}
...
file <- here("03_data/990_processed_data/008_pseudotime",
             "all_transcription_factors_pericyte_lineage.csv")
