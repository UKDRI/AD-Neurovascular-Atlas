...
all <- x |> dplyr::filter(set_name == "Perivascular-FB-KAZN2") |>
  ...
  gwas_backgroup = unique(sig_celltypes_magma$data[[1]]$gwas_background)) |>
  dplyr::select(celltype, input_gene_list, gwas_backgroup, percent, data)
...
ad_all_controls <- list(
  "Microglia-activated_sig-magma-sig-genes_EUROPEUKBB_35k10k_all_controls_top-ten-percent" = c("1" = "Lymphocyte activation", "2" = "Amyloid", "3" = "Dephosphorylation"),
  "Pericyte-2_sig-magma-sig-genes_EUROPEUKBB_35k10k_all_controls_top-ten-percent" = c("1" = "Amyloid", "3" = "Supramolecular fibre organisation"),
  "Perivascular-FB-KAZN2_sig-magma-sig-genes_EUROPEUKBB_35k10k_all_controls_top-ten-percent" = c("2" = "Amyloid", "4" = "Viral process", "1" = "Axon terminus")
)
...
ppi_network_plots <- function(group, module_df, overall_network = my_network) {
  ...
  module_legend <- df |>
    dplyr::select(module_label, module_color) |>
    dplyr::distinct()
...
plot(subgraph, 
     ...
     main = group)
  # Add legend
  legend("bottomright", legend = unique(df$module_label), col = unique(df$module_color), pch = 19, title = "Module")
 box()
 recordPlot()
...
V(subgraph)$hgnc_symbol <- ensembl_to_symbol[V(subgraph)$name]
  ...
  # Plot using ggraph
  p <- ggraph(tbl_graph, layout = 'fr') +  # 'fr' layout for Fruchterman-Reingold
...
plots <- map(unique(modules$label), plot_network_ggraph, modules)
