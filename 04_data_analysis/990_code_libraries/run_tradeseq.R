...
if (!file.exists(file)) {
  ...
  qs::qsave(sce_subsampled, str_replace(file, "_sds_", "_"))
  
} else {
  sds <- qs::qread(file)
  sce_subsampled <- qs::qread(str_replace(file, "_sds_", "_"))
}
...
# Continue with variance estimation on filtered data
var_genes <- modelGeneVar(sce_subsampled)
top_hvgs <- getTopHVGs(var_genes, n = 2000)
