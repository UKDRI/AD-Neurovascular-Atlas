...
#| eval: true
# List files
files <- list.files(here::here("03_data/994_magma_inputs/results"), 
...
df$percent <- regmatches(string, regexpr(pattern_3, string))
...
get_heatmap_data <- function(df) {
  ...
  heatmap_data <- df |>
    ...
    pivot_wider(names_from = VARIABLE, values_from = log_p) |>
    as.data.frame()
...
```{r}
subset_column <- heatmap_data_no_scz$`level2_top-ten-percent`[, "all_controls_Bellenguez_2022_35k10k", drop = FALSE]
colnames(subset_column) <- "Bellenguez_2022"
head(subset_column)
heatmap_for_paper <- make_heatmap(subset_column, "")
heatmap_for_paper

qs::qsave(heatmap_for_paper, here::here("05_figures/990_shared_figures/003_final_figures/magma_celltype_heatmap.qs"))
qs::qsave(subset_column, here::here("05_figures/990_shared_figures/003_final_figures/magma_celltype_heatmap_data.qs"))
```
...
# Get heatmaps
heatmap_data_no_filter <- heatmap_data[1:2]
...
hlist <- heatmaps_to_join$level1_no_filter %v% heatmaps_to_join$level2_no_filter
draw(hlist)
...
ppi_magma <-
  left_join(ppi_magma,
            module_labels,
            by = join_by(celltype_module, percent, input_gene_list == fraction))
```
...
# Add Gene symbols and entrezgene_ids
ad_level2 <- ad_level2 |>
  dplyr::left_join(id_translate, by = join_by(GENE == entrezgene_id))
```

```{r}
# get unmatched ID symbols
missing_ids <- ad_level2$GENE[is.na(ad_level2$gene)] |> unique()
ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")
mapped_genes <- getBM(attributes = c('entrezgene_id', 'ensembl_gene_id', 'hgnc_symbol'), 
...
ad_level2 <- ad_level2 |>
  ...
  dplyr::mutate(ensembl_gene_id = ifelse(is.na(ensembl_gene_id.x), ensembl_gene_id.y, ensembl_gene_id.x), hgnc_symbol = ifelse(is.na(gene), hgnc_symbol, gene)) |>
  dplyr::select(!c(gene, ensembl_gene_id.x, ensembl_gene_id.y, group, gene_set))
...
readr::write_tsv(unique_genes, here::here("03_data/994_magma_inputs/sig_genelist", "unique_sig_magma_set_genes.tsv"))
```
