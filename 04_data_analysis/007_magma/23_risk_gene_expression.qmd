...
res_sig_level2 <- res |>
  ...
  dplyr::select(entrezgene_id = GENE, padj, celltype) |>
  dplyr::left_join(translated_id, by = "entrezgene_id") |>
  readr::write_csv(here::here(
    "03_data/990_processed_data/007_magma/level2_sig_magma_genes.csv"
  ))
```
...
sce_sub <- subset(
  ...
  subset = diagnosis == "Control" &
    sce$highlevel_manual_annotations %in% unique(res_sig$celltype)
)
Idents(sce_sub) <- sce_sub$highlevel_manual_annotations
# Check if all genes of interest are in the object
genes_of_interest <- setdiff(
  unique(res_sig$ensembl_gene_id),
  rownames(sce_sub@assays$RNA@data)
)
...
assertthat::are_equal(
  res_sig$hgnc_symbol[res_sig$hgnc_symbol %in% genes_sub$gene],
  genes_sub$gene
)
res_sig$ensembl_gene_id[
  res_sig$hgnc_symbol %in% genes_sub$gene
] <- genes_sub$ensembl
...
genes_of_interest_symbols <- gene_ensembl_ids$gene[
  gene_ensembl_ids$ensembl %in% genes_of_interest
]
...
ggsave(
  here::here("05_figures/990_shared_figures/001_magma/risk_gene_heatmap.png"),
  risk_gene_heatmap,
  height = 10
)
ggsave(
  here::here("05_figures/990_shared_figures/001_magma/risk_gene_dotplot.png"),
  risk_gene_dotplot,
  height = 10
)
```
...
plot <- Heatmap(expression_matrix, name = "Expression", column_names_rot = 45)
png(
  here::here(
    "05_figures/990_shared_figures/001_magma/risk_gene_heatmap_mean_level1.png"
  ),
  width = 6,
  height = 10,
  units = "in",
