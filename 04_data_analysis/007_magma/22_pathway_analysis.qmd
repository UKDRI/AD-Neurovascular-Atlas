...
readr::write_csv(
  res_sig_level2,
  "03_data/994_magma_inputs/sig_genelist/magma_level2_controls_sig_genes.csv"
)
...
names(genes) <- paste0(
  genes_df$celltype,
  "-",
  genes_df$gwas_background,
  "-",
  genes_df$celltype_level
)
```
...
readr::write_tsv(
  df,
  here(
    "03_data/999_data_for_publication/case_risk/risk_genes_case_vs_control.tsv"
  )
)
...
map2(
  ...
  ~ ggsave(
    here::here(
      "05_figures/990_shared_figures/001_magma/magma_pathways",
      paste0(.y, "_sig_genes_only", ".png")
    ),
    plot = .x,
    width = 32,
    height = 10
...
map2(
  ...
  ~ ggsave(
    here::here(
      "05_figures/990_shared_figures/001_magma/magma_pathways",
      paste0(.y, "_sig_genes_only-dotplots", ".png")
    ),
    plot = .x,
    width = 26,
    height = 12
...
merged_go_plot <- function(celltype, go_results_list) {
  ...
  combined_df <- do.call(
    rbind,
    lapply(names(gos), function(name) {
...
rankings <-
  map2(
    gsea_df$data,
    gsea_df$celltype,
    ~ (sort_rankings(.x$ZSTAT, .x$P, .x$hgnc_symbol, .y))
  ) |>
  purrr::set_names(paste0(
    gsea_df$celltype,
    "-",
    gsea_df$gwas_background,
    "-",
    gsea_df$celltype_level
  ))
```
...
ggplot(
  data.frame(
    gene_symbol = names(
      rankings$`Microglia-activated-case_EUROPEUKBB_35k10k-level2`
    )[1:50],
  ...
  ggtitle(
    "Microglia-activated top 50 ranked genes",
...
named_vectors <- res_sig |>
  ...
  dplyr::mutate(
    gene_list = map(
      data,
      ~ setNames(.x$ZSTAT, .x$hgnc_symbol) |>
...
if (include_apoe) {
  readr::write_csv(
    insulin_genes,
    here::here("insulin_celltypes_sig_genes_apoe.csv")
  )
} else {
  readr::write_csv(
    insulin_genes,
    here::here("insulin_celltypes_sig_genes_no-apoe.csv")
  )
}
```
