...
top_endomt <- all_markers |>
  dplyr::left_join(translated_id, by = join_by(gene == ensembl_gene_id)) |>
  dplyr::filter(grepl("EndoMT", cluster) & p_val_adj == 0 & hgnc_symbol != "") |>
  slice_max(avg_log2FC, n = 5, by = cluster)

rownames(sce@assays$RNA@data) <- gene_ensembl_ids$gene
plot <- DotPlot(sce, features = top_endomt$hgnc_symbol)
plot
ggsave(here::here("05_figures/990_shared_figures/endomt_top_markers.png"), height = 8, width = 12)
```
