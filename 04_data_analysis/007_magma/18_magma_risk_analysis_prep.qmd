...
```{r}
write_csv(top_genes$ten, here::here("top_10_percent_celltype_markers.csv"))
```

```{r}
#| eval: false

# Look online for list of transition markers and see if they're here or perhaps in their expression?
trans_endo_mural_markers <- top_genes$ten |> dplyr::filter(cluster == "Trans-Endo-to-mural")

ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")
entrez_ids <- getBM(attributes = c('ensembl_gene_id', 'external_gene_name'),
                    filters = 'ensembl_gene_id',
                    values = trans_endo_mural_markers$gene,
                    mart = ensembl)
# Remove rows with NA Entrez IDs if present
entrez_ids <- entrez_ids[!is.na(entrez_ids$external_gene_name), ]

# Get unique Ensembl-Entrez pairs by keeping the first occurrence
entrez_ids <- entrez_ids[!duplicated(entrez_ids$ensembl_gene_id), ]

trans_endo_mural_markers <- trans_endo_mural_markers |>
  dplyr::left_join(unique(entrez_ids), by = join_by(gene == ensembl_gene_id))
```
...
map2(formatted_data, names(formatted_data), ~ readr::write_tsv(.x,
                 here::here(
                   ...
                   paste0("magma_celltypes_", level, "_", subset, "_top-", .y, "-percent.magma.txt")
                 ),
                 col_names = FALSE))
```
  ...
  map2(
    ...
    ~ readr::write_tsv(
      ...
      here::here(
        ...
        paste0("sig_celltype_", level, "_", subset, "_", .y, ".condition.txt")
      ),
      col_names = FALSE
    )
  )
...
#| eval: false
#### Search for datasets ####
metagwas <- MungeSumstats::find_sumstats(traits = c("parkinson","alzheimer", "stroke", "schizophrenia", "vascular dementia", "white matter hyperintensities"), 
