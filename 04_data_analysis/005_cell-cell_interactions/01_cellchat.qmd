...
plot <- netVisual_bubble(
  cellchat_control,
  sources.use = parenchymal,
  targets.use = vascular_all,
  signaling = c("EGF"),
  remove.isolate = TRUE
) +
  ggtitle("Parenchymal -> Vascular EGF Signalling in controls")
plot

filename <- here::here(
  "05_figures/990_shared_figures/cellchat",
  "egf_signalling_controls.pdf"
)
ggsave(filename)
```
...
### EGF differential signalling

```{r}
cellchat_all <- identifyOverExpressedGenes(
  cellchat,
  group.dataset = "datasets",
  pos.dataset = pos.dataset,
  features.name = features.name,
...
net_all <- netMappingDEG(
  cellchat_all,
  features.name = features.name,
  variable.all = TRUE
)
# Filter for EGF-related pathways
egf_ligands <- c("EGF", "AREG", "TGFA", "NRG1", "NRG2", "NRG3", "NRG4")
egf_receptors <- c("EGFR", "ERBB2", "ERBB3", "ERBB4")

egf <- net_all |>
  dplyr::filter(ligand %in% egf_ligands)
head(egf)

pairLR.use.up <- egf[, "interaction_name", drop = F]

gg1_v <- netVisual_bubble(
  cellchat,
  pairLR.use = pairLR.use.up,
  sources.use = vascular_all,
  targets.use = vascular_all,
  comparison = c(1, 2),
  angle.x = 90,
  remove.isolate = TRUE,
...
filename <- here::here(
  "05_figures/990_shared_figures/cellchat",
  "egf_differential_signalling_vascular.pdf"
)
ggsave(filename, gg1_v)
...
filename <- here::here(
  "05_figures/990_shared_figures/cellchat",
  "egf_differential_signalling_vascular-parenchymal.pdf"
)
ggsave(filename, gg1_v)
...
filename <- here::here(
  "05_figures/990_shared_figures/cellchat",
  "egf_differential_signalling_parenchymal-vascular.pdf"
)
ggsave(filename, gg1_v)
gg1_v + gg2_v
```
