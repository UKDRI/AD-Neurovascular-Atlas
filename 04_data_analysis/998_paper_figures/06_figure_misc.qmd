...
#| eval: false
knitr::purl(
  "04_data_analysis/998_paper_figures/04_figures.qmd",
  "temp-dotplot.R"
)
```
...
ggplot(
  cell_props,
  aes(x = apoe_status, y = proportion, fill = abbreviated_idents)
) +
...
readr::write_csv(
  endomt_percent,
  here::here("03_data/999_data_for_publication", "endomt_level1_percent.csv")
)
```
...
readr::write_csv(
  endomt_nuclei_num,
  here::here("03_data/999_data_for_publication", "endomt_nuclei_num.csv")
)
```
...
assertthat::assert_that(
  sum(!(unique(Idents(sce)) %in% unique(sce$abbreviated_idents))) == 0
)
file <- here::here(
...
level2_dotplot <- DotPlot(sce, features = unique(top_markers$hgnc_symbol)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(x = "", y = "")
level1_dotplot <- DotPlot(
  sce,
...
ggsave(
  here(
    "05_figures/990_shared_figures/003_final_figures/000_sup_figures/level2_dotplot.pdf"
  ),
  plot = level2_dotplot,
  width = 15,
  height = 9
...
readr::write_tsv(
  meta_data,
  here::here("03_data/999_data_for_publication/nuclei_meta_data.tsv")
)
...
bar_plot <- ggplot(
  percentages,
  aes(
    x = reorder(abbreviated_idents, -percentage * (prep == "V")),
    y = percentage,
    fill = prep
  )
) +
  ...
  labs(x = "", y = "Percentage", fill = "Fraction") +
...
# Save the plot
ggsave(
  here("05_figures/990_shared_figures/003_final_figures/bar_plot_P_vs_V.png"),
  plot = bar_plot,
  width = 6,
  height = 4,
  dpi = 300
...
ggsave(
  here("05_figures/990_shared_figures/003_final_figures/bar_plot_P_vs_V.svg"),
  plot = bar_plot,
  width = 6,
  height = 4
)
...
num_nuclei |>
  ...
  labs(x = NULL, y = "Number of Nuclei")
...
ggsave(
  filename = here::here(
    "05_figures/990_shared_figures/003_final_figures",
    "000_sup_figures/cohort_boxplot.png"
  ),
  plot = age_plot,
  width = 8,
  height = 8,
...
ggsave(
  filename = here::here(
    "05_figures/990_shared_figures/003_final_figures/figure4.png"
  ),
  plot = final_plot,
  width = 12,
  height = 16,
