---
title: Get VCF specific position - set 1
execute: 
  eval: false
---

We start by converting the plink export of the genotype data to VCF:
...
plink --file October2023 --recode vcf --out Webber_Neurochip_2023_chr --output-chr "chrM"
```

Then we split the complete VCF into one VCF per sample:
...
VCF_FILE="/scratch/scw1329/gmbh/blood-brain-barrier-in-ad/03_data/992_genotyping_data/PLINK_021123_0956/Webber_Neurochip_2023_chr.vcf"
OUT_DIR="/scratch/scw1329/gmbh/blood-brain-barrier-in-ad/03_data/992_genotyping_data/PLINK_021123_0956/split_samples/"
...
Note that the Alex on the lab side mentioned one sample having signs of being low quality, but luckly this one was repeated as there're 24 wells on the chip and only 20 samples, so 4 got repeats.
The specific sample was NP102_17, but the repeat NP102_17R was fine.

```{r}
#| purl: false
# remove poor quality sample
rm /scratch/scw1329/gmbh/blood-brain-barrier-in-ad/03_data/992_genotyping_data/PLINK_021123_0956/split_samples/4_NP102_17.vcf
```


We also need a liftover file to translate between hg19 and GRCh38 cellranger outputs.
Frank already created this so I'll just reuse it.
...
I thought I'd be able to figure out an elegant way of using this one script with arguments, but realised I need to be able to adjust the array number to the number of samples per set, which I don't see how to do...
