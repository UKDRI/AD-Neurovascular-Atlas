...
mast_de |>
  ...
  readr::write_tsv(here(
    "03_data/990_processed_data/001_snrnaseq/13_mast_de",
    mast_level,
    "mast_sig_deg_numbers.tsv"
  ))
```

```{r}
mast_de |>
  filter(hgnc_symbol == "EGF") |>
  readr::write_tsv(here(
    "03_data/990_processed_data/001_snrnaseq/13_mast_de",
    mast_level,
    "mast_egf.tsv"
...
compare_de_results <- function(
  ...
  spearman_correlation <- cor(
    merged_results[[paste0("rank_", de1_name)]],
    merged_results[[paste0("rank_", de2_name)]],
    method = "spearman"
  )

  overlap_genes <- intersect(df1$gene, df2$gene)
  cat(
    "Number of overlapping genes for",
    cell_type,
    ":",
...
df <- map_dfr(
  ...
  dplyr::rename(
    n_total_pseudobulk = method_1_n_degs,
...
jaccard_matrix <- matrix(
  unlist(jaccard_values),
  nrow = length(common_celltypes),
...
png(
  here::here(
    ...
    "mast_pseudobulk_jaccard_heatmap.png"
  ),
  width = 4000,
  height = 4000,
  res = 300
...
```{r}
go_results_all_mast <- future_map(
  genes_mast,
  ~ enrichGO(
    gene = .x,
    OrgDb = "org.Hs.eg.db",
...
if (mast_level != "level1" & !filter_logfc) {
  qs::qsave(
    go_results_filtered,
    here::here(file, "go-hypergeometic_subtypes_deg_direction_mast_level2.qs")
  )
} else if (mast_level == "level1" & !filter_logfc) {
  qs::qsave(
    go_results_filtered,
    here::here(file, "go-hypergeometic_subtypes_deg_direction_mast_level1.qs")
  )
} else if (mast_level != "level1" & filter_logfc) {
  qs::qsave(
    go_results_filtered,
  ...
  qs::qsave(
    ...
    here::here(
      file,
      "go-hypergeometic_subtypes_deg_direction_mast_level1_logfc_filter.qs"
    )
  )
}
#go_results_filtered <- qs::qread(file)
...
end1 |>
  ...
  readr::write_csv(here::here(
    "03_data/990_processed_data/001_snrnaseq/09_pathway_analysis/endomt1_up_pathway_gene_expression.csv"
  ))
end1 |>
  dplyr::left_join(expanded_df, by = join_by(hgnc_symbol == geneID)) |>
  dplyr::filter(grepl("endotheli", Description, ignore.case = TRUE)) |>
  readr::write_csv(here::here(
    "03_data/990_processed_data/001_snrnaseq/09_pathway_analysis/endomt1_up_pathway_gene_expression_endeothelial_subset.csv"
  ))
```
...
ggplot(
  ...
  ggtitle("Arterial top 50 ranked genes", subtitle = "Control VS AD")
```
...
if (include_apoe) {
  readr::write_rds(
    dotplots,
    here::here(
      "03_data/990_processed_data/001_snrnaseq/09_pathway_analysis/go-dotplots_subtypes_apoe.rds"
    )
  )
} else {
  readr::write_rds(
    dotplots,
    here::here(
      "03_data/990_processed_data/001_snrnaseq/09_pathway_analysis/go-dotplots_subtypes_no-apoe.rds"
    )
  )
}
```
...
map2(
  ...
  ~ ggsave(
    here::here(
      "05_figures/990_shared_figures/002_pseudobulk/gsea",
      paste0(.y, ".png")
    ),
    plot = .x,
    width = 18,
