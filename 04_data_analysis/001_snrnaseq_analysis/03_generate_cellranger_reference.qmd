...
In order to run the [`cellranger`](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/what-is-cell-ranger) pipeline one needs a reference genome for it to align to.
10X do provide a human reference, but it's rather old at time of writing, so here we'll generate a more recent one.

Note that the initial download of the reference files are to be done on the login node.
I'll save them in the shared Webber dir on gluster (`/gluster/dri02/rdscw/shared/webber/reference_genomes`) in case others want to use it.
