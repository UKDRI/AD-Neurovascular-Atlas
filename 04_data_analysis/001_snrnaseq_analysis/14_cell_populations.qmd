...
Let's try a chi-squared test to look for any significant differences in celltypes between cases and controls

## Level 1 celltypes

```{r}
# Create the contingency table
contingency_table <- table(sce$diagnosis, sce$highlevel_manual_annotations)

# Perform the chi-squared test
chi_squared_test_result <- chisq.test(contingency_table)

# If you expect some cell types to have very small counts, use Fisher's exact test instead
# fisher_test_result <- fisher.test(contingency_table)

# View the test result
print(chi_squared_test_result)

# If using Fisher's test
...
```{r}
# Create the contingency table
contingency_table <- table(sce$diagnosis, sce$subcelltype_annotations)
# Perform the chi-squared test
chi_squared_test_result <- chisq.test(contingency_table)
# View the test result
print(chi_squared_test_result)
...
-   NOTE: need to add checks for normalcy
-   NOTE: not working on render - need to debug

```{r}
#| eval: false
# Get unique cell types
unique_cell_types <- unique(Idents(sce))

# Create a function to perform the tests and adjust p-values
perform_tests <- function(cell_type) {
  # Subset cell counts for the current cell type in AD and control groups
  cell_counts_AD <- sce$cell_counts[sce$group == "AD" & sce$cell_type == cell_type]
  cell_counts_control <- sce$cell_counts[sce$group == "control" & sce$cell_type == cell_type]
  
  # Perform t-test
  ttest_result <- t.test(cell_counts_AD, cell_counts_control)
  ttest_pvalue <- ttest_result$p.value
  
  # Perform Wilcoxon rank-sum test
  wilcox_result <- wilcox.test(cell_counts_AD, cell_counts_control)
  wilcox_pvalue <- wilcox_result$p.value
  
  # Return adjusted p-values
  return(list(CellType = cell_type, TTest_PValue = ttest_pvalue, Wilcox_PValue = wilcox_pvalue))
}

# Apply the function to each unique cell type using map
test_results <- map(unique_cell_types, perform_tests)

# Extract p-values
ttest_pvalues <- map_dbl(test_results, "TTest_PValue")
wilcox_pvalues <- map_dbl(test_results, "Wilcox_PValue")

# Adjust p-values using the Benjamini-Hochberg procedure
ttest_pvalues_adjusted <- p.adjust(ttest_pvalues, method = "BH")
wilcox_pvalues_adjusted <- p.adjust(wilcox_pvalues, method = "BH")

# Create a dataframe with the results
result_df <- data.frame(CellType = unique_cell_types, TTest_PValue = ttest_pvalues, TTest_PValue_Adjusted = ttest_pvalues_adjusted, Wilcox_PValue = wilcox_pvalues, Wilcox_PValue_Adjusted = wilcox_pvalues_adjusted)
print(result_df)
```
...
The lab works on very high throughput stuff, so I guess it's intended for bigger sample sizes.
...
ccs_v = new_cell_count_set(cds_v, 
                         sample_group = "manifest", 
                         cell_group = "subcelltype_annotations")
ccs_p = new_cell_count_set(cds_p, 
                         sample_group = "manifest", 
                         cell_group = "subcelltype_annotations")

ccm_v  = new_cell_count_model(ccs_v,
                            main_model_formula_str = "~ diagnosis")
ccm_p  = new_cell_count_model(ccs_p,
                            main_model_formula_str = "~ diagnosis")
...
```{r}
#| eval: false
avg_exp <- AverageExpression(sce, group.by = c("highlevelcelltype_annotations", "donor"))
avg_exp <- avg_exp$RNA
...
# add in case status
avg_exp <- sce@meta.data |> 
  dplyr::distinct(donor, .keep_all = TRUE) |> 
  dplyr::select(donor, diagnosis) |>
  dplyr::left_join(avg_exp)

readr::write_rds(avg_exp, here::here("03_data/990_processed_data/001_snrnaseq/11_cell_network_interactions/average-expression_per-donor_highlevel-celltype.rds"))
```
