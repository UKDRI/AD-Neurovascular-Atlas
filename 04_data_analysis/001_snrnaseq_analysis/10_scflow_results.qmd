...
-   programuline + receptor - check in DE
-   order cellchat plots - vascualr and parenchymal
-   Do overlap for the MAST/PSEudoDEs 
-   Get nuclie numbers/celltype proportions from Yang/Mattews and rosmap
# Load packages
...
```{r}
VlnPlot(seurat, features = c("nFeature_RNA", "nCount_RNA", "pct_mito", "pct_ribo"), ncol = 1, group.by = "prep", pt.size = 0)
VlnPlot(seurat, features = c("nFeature_RNA", "nCount_RNA", "pct_mito", "pct_ribo"), ncol = 1, group.by = c("prep", "diagnosis"), pt.size = 0)
```
...
#seurat <- readr::read_rds(here::here("03_data/990_processed_data/001_snrnaseq/07_scflow_analysis/scflow-seurat-postprocessing-filtered.rds"))
#seurat <- readr::read_rds(here::here("03_data/990_processed_data/001_snrnaseq/07_scflow_analysis/scflow-seurat-postprocessing_donors-excluded.rds"))
seurat <- readr::read_rds(here::here("03_data/990_processed_data/001_snrnaseq/07_scflow_analysis/scflow-seurat-postprocessing_donors-excluded-updated.rds"))
...
Expected doublet rates on 10X site [here](https://kb.10xgenomics.com/hc/en-us/articles/360001378811-What-is-the-maximum-number-of-cells-that-can-be-profiled).
Note the code for running `DoubletFinder` is in the `_targets.R` file.


```{r}
#| eval: false
tar_load(seurat_doublets)

# join celltype labels from filtered sce to this one and check singlet numbers
table(Idents(seurat_doublets))

x = merge(sce@meta.data, seurat_doublets@meta.data, by = "row.names")
table(x$highlevel_manual_annotations, x$doublet_estimation)
```
...
sweep_res_list <- paramSweep_v3(seurat, PCs = 1:35, sct = TRUE, num.cores = 6)
...
# sweep_res_list <- paramSweep_v3(seu_kidney, PCs = 1:10, sct = FALSE)
# gt.calls <- seu_kidney@meta.data[rownames(sweep_res_list[[1]]), "GT"].   ## GT is a vector containing "Singlet" and "Doublet" calls recorded using sample multiplexing classification and/or in silico geneotyping results 
# sweep_stats <- summarizeSweep(sweep_res_list, GT = TRUE, GT.calls = gt.calls)
# bcmvn_kidney <- find.pK(sweep_stats)
...
#| eval: false
library(Azimuth)
sce <- RunAzimuth(seurat, reference = "humancortexref")
...
umap_batch
# plot umap clusters
umap_clusters <- DimPlot(seurat, reduction = "umap", label = TRUE, raster = FALSE)
umap_clusters
...
ggsave(here::here("05_figures/990_shared_figures/scflow_umap_clusters.png"), umap_clusters, device = "png")
# clean up plots
rm(umap_diag, umap_prep, umap_celltype, umap_batch, umap_clusters)
```
...
#readr::write_csv(cluster_markers, here::here("03_data/990_processed_data/001_snrnaseq/07_scflow_analysis/cluster_markers.csv"))
...
celltypes <- c("0" = "Oligo", "1" = "Capillary", "2" = "T-Peri", "3" = "Arterial", 
               "4" = "Astro", "5" = "vascular-SMC", "6" = "Oligo", "7" = "Trans-Endo-to-mural", 
...
Three of the candidate genes look good, check the expression in cases VS controls for those three

```{r}
# Vector of promising targets based on celltype expression
promising_targets <- c("SLC7A1", "ELOVL7", "ATP10A")

# Plot cases VS control
bbb_marker_plot <- VlnPlot(sdata_endo, features = promising_targets, ncol = 2, pt.size = 0, split.by = "diagnosis") + 
  theme(legend.position = "right")
bbb_marker_plot
ggsave(here::here("05_figures/990_shared_figures/bbb_marker_subset_expression_levelone_case_vs_control.png"), bbb_marker_plot, device = "png")
```
...
<!-- Remove the ambiguous cells - they could well be doublets and there aren't enough of them to worry about keeping them. -->

```{r}
#| include: false
#| eval: false
sdata_endo
rownames(sdata_endo@assays$RNA@data) <- ensembl_to_symbol$ensembl_gene_id
sdata_endo <- subset(sdata_endo, idents = "ambiguous", invert=TRUE)
# paste ensembl id and gene id to keep unique rownames
rownames(sdata_endo@assays$RNA@data) <-
  paste0(ensembl_to_symbol$hgnc_symbol,
         "_",
         ensembl_to_symbol$ensembl_gene_id)
sdata_endo
```
...
new_labels <- str_replace_all(current_labels, "Capillary|Venous|Arterial", "Endothelial") %>%
  str_replace_all("T-Peri", "Pericyte") %>%
  str_replace_all("vascular-SMC", "SMC") %>%
  str_replace_all("Oligo-pre", "OPC") %>%
  str_replace_all("Trans-Endo-to-mural", "mystery-cluster") %>%
  str_replace_all("Peri-Fibro|Meningeal-Fibro", "Fibroblast")

names(new_labels) <- current_labels
sdata_endo$manual_annotations <- Idents(sdata_endo)
sce <- RenameIdents(sdata_endo, new_labels)
sdata_endo$highlevel_manual_annotations <- Idents(sdata_endo)
```
...
I have the genotyped data now and have computed the APOE status from the probes.
Note that the data isn't precise on which allele is which, so there is an instance of an unknown that is almost certainly due to the alleles being different for each of the two SNPs, so that will need to be corrected

```{r}
# read in apoe status table
apoe_status <- readr::read_csv(here::here("03_data/992_genotyping_data/apoe_status.csv"))

# Correct case of unknown apoe status
apoe_status <- apoe_status |>
  dplyr::mutate(apoe_status = ifelse(rs7412 == "0/1" & rs429358 == "0/1", "E2/E4", apoe_status))
# check numbers
table(apoe_status$apoe_status)

# Check repeated samples
# the unique sample ID is between the first and second _
apoe_status$unique_id <- sapply(strsplit(as.character(apoe_status$sample_name), "_"), function(x) x[2])
...
So it seems there's disagreement for sample NP160...
Having checked the VCF files for each, I can see that one probe is 1/1 and the other ./. (denoting uncertainty) for `3_NP160_13`, whereas both probes from `23_NP160_13R` are 0/1.
Given that E3 is more common, it seems more reasonable to assume this sample is E3/E4.

```{r}
apoe_status <- apoe_status |>
  # remove inaccurate replicate
  dplyr::filter(sample_name != "3_NP160_13") |>
  # keep one of the remaining replicates
  dplyr::distinct(unique_id, .keep_all = TRUE) |>
  # remove the "E" from the apoe status to match the current meta.data format
  dplyr::mutate(apoe_status = gsub("E", "", apoe_status))
```

Now I need the sample IDs of the neurochips to match the APOE status to the sce meta.data

```{r}
# Get sample IDs
neurochip_ids <- readr::read_csv(here::here("03_data/990_processed_data/001_snrnaseq/90_sample_info/neurchip_sample_ids.csv")) |>
  dplyr::mutate(donor = paste0("donor-", str_extract(fraction_donor, "(?<=_).*"))) |>
  dplyr::select(-fraction_donor) |>
  dplyr::distinct()

apoe_status <- apoe_status |>
  dplyr::left_join(neurochip_ids, by = c("sample_name" = "neurochip")) 

apoe_status <- apoe_status |>
  # select only the apoe_status and donor columns to merge
  dplyr::select(apoe_status, donor)

# Merge the new APOE status information with the existing meta.data
# Use all.x = TRUE to keep all samples in the original meta.data
updated_meta <- merge(sce@meta.data, apoe_status, by.x = "donor", by.y = "donor", all.x = TRUE)
updated_meta <- dplyr::left_join(sce@meta.data, apoe_status, by = "donor")
# Lost rownames, add them back
# First check that order of rows hasn't changed
assertthat::assert_that(sum(sce@meta.data$nCount_RNA != updated_meta$nCount_RNA) == 0)
rownames(updated_meta) <- rownames(sce@meta.data)
# merge the two apoe columns
updated_meta <- updated_meta %>%
  mutate(apoe_status = coalesce(APOE, apoe_status)) %>%
# remove old column
  select(-APOE)  

# update meta.data
sce@meta.data <- updated_meta
rm(updated_meta, apoe_status, neurochip_ids)
```
